...
完全に静的な Envoy から、Kubernetes 上の 2 ポッドメッシュまで登る 4 つのハンズオン。
Envoy データプレーン側はほとんど変わらない — 変わるのは**設定の配信方法**だ。それこそが
xDS の学びのすべて。
...
- コントロールプレーンのイメージ再ビルドには `go` 1.24+ が要るが、`docker compose up --build`
  の中で Docker が代わりにやってくれる。

## 約束ごと

- Envoy の管理インターフェースは全ラボで `:9901`。状態確認はまずここから:
  `/config_dump`, `/clusters`, `/listeners`, `/stats`。
- [`../scripts`](../scripts) のヘルパースクリプトが、よく使う admin クエリをラップしている。
- 各ラボ README の末尾に **片付け（Teardown）** がある。次に進む前に実行して、ポートと
  Docker ネットワークを解放すること。
