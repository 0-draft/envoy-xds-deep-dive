...
```mermaid
flowchart LR
    accTitle: Lab 02 トポロジ
    accDescr: go-control-plane のコントロールプレーンがポート 18000 で ADS gRPC（LDS/RDS/CDS/EDS）を Envoy に配り、HTTP admin は 19000。curl は 10000 で Envoy に届き、Envoy は EDS で upstream1 と upstream2 にロードバランスする。
